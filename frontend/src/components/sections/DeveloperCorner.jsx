...
export const DeveloperCorner = ({ profile }) => {
  const defaultCapabilities = [
    {
      id: '01',
      title: 'Microservices & Distributed REST API Architecture',
      spec: 'Java 21 • Spring Boot 3 • OpenFeign • Resilience4j',
      targetId: 'projects',
    },
    {
      id: '02',
      title: 'Relational Schema Modeling & SQL Query Optimization',
      spec: 'MySQL • PostgreSQL • Hibernate / JPA • Index Strategies',
      targetId: 'skills',
    },
    {
      id: '03',
      title: 'Stateless Authentication, OAuth2 & Security Hardening',
      spec: 'Spring Security 6 • JWT Tokens • RBAC Authorization',
      targetId: 'skills',
    },
    {
      id: '04',
      title: 'Containerized Deployment & Production Orchestration',
      spec: 'Docker • Multi-stage Builds • Compose • Linux Env',
      targetId: 'projects',
    },
    {
      id: '05',
      title: 'Distributed Caching & Transactional Consistency',
      spec: 'Redis • @Transactional Boundaries • Optimistic Locking',
      targetId: 'skills',
    },
    {
      id: '06',
      title: 'Full-Stack Reactive UI & REST API Integration',
      spec: 'React 18 • Responsive Design • Clean State Management',
      targetId: 'projects',
    },
  ];
  ...
  if (profile?.devCornerCapabilities) {
    try {
      ...
      if (lines.length > 0) {
        capabilities = lines.map((line, idx) => {
          const parts = line.split('|').map((s) => s.trim());
          return {
            id: String(idx + 1).padStart(2, '0'),
            title: parts[0] || 'Competency',
            spec: parts[1] || 'Core Specification',
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {profile?.inFocusTitle || 'Enterprise Microservices Architecture & High-Throughput Engine'}
                </h4>
                ...
                >
                  {profile?.inFocusDescription || 'Designed and deployed a resilient microservices foundation with Java 21 and Spring Boot 3. Implements stateless JWT authentication, distributed transactional boundaries, asynchronous order processing, and defensive circuit breakers for high availability under heavy transactional load.'}
                </p>
                ...
                <div className="in-focus-metrics-grid">
                  <div>
                    <div style={{ fontSize: '1.15rem', fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>
                      {profile?.inFocusMetric1Value || '99.9%'}
                    </div>
                    <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      {profile?.inFocusMetric1Label || 'SLA Uptime'}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '1.15rem', fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>
                      {profile?.inFocusMetric2Value || '<85ms'}
                    </div>
                    <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      {profile?.inFocusMetric2Label || 'P95 Latency'}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '1.15rem', fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>
                      {profile?.inFocusMetric3Value || 'Zero'}
                    </div>
                    <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      {profile?.inFocusMetric3Label || 'Session State'}
                    </div>
                  </div>
                </div>
              </div>
